...
class BLIPModelWrapper:
    def __init__(self, root_dir, device, variant="blip-flickr-base"):
        ...
        if not (
            ...
            and os.path.exists(self.bert_config_path)
        ):
            self.download()
    ...
    @torch.no_grad()
    def get_image_embeddings(self, image: Union[PIL.Image.Image, List[PIL.Image.Image]]):
        if isinstance(image, PIL.Image.Image):
            image = [image]  # Convert single image to list

        image_tensor = torch.stack([image_preprocess(img) for img in image], dim=0).to(self.device)
        image_feat = self.model.visual_encoder(image_tensor)
        image_embed = self.model.vision_proj(image_feat[:, 0, :])
        image_embed = F.normalize(image_embed, dim=-1)
        return image_embed

    @torch.no_grad()
    def get_image_embeddings_dataloader(self, image_loader):
        image_feats = []
        image_embeds = []
    ...
    def get_retrieval_scores_dataset(self, loader):
        ...
        for i, sims in enumerate(metric_logger.log_every(sims_matrix[start:end], 50, "Evaluation T2i")):
            ...
            encoder_att = torch.ones(encoder_output.size()[:-1], dtype=torch.long).to(self.device)
            output = self.model.text_encoder(
                text_ids[start + i].repeat(self.config["k_test"], 1),
    ...
    def run_scores_batched(self, image_embeds, image_feats, text_embeds, text_ids, text_atts):
        ...
        sims_matrix = torch.einsum(
            "ijk,ilk->ijl", image_embeds, text_embeds
        )  # (n_tests, n_image_options, n_text_options)
        ...
        for i, sims in enumerate(sims_matrix):
            for j in range(sims.shape[0]):
                ...
                encoder_att = torch.ones(encoder_output.size()[:-1], dtype=torch.long).to(self.device)
                output = self.model.text_encoder(
                    text_ids[i, j].repeat(sims_matrix.shape[2], 1),
    ...
    def get_retrieval_scores_batched(self, joint_loader):
        ...
        for batch in tqdm(joint_loader):
            ...
            for c_option in batch["caption_options"]:
                ...
                text_input = self.model.tokenizer(
                    c_option, padding="max_length", truncation=True, max_length=35, return_tensors="pt"
                ).to(self.device)
                text_output = self.model.text_encoder(
                    text_input.input_ids, attention_mask=text_input.attention_mask, mode="text"
                )
